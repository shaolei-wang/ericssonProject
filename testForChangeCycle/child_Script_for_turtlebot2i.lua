function sysCall_init()
    ...
    leftMotor=sim.getObjectHandle("dr20_dynamicLeftWheel") -- Handle of the left motor
    rightMotor=sim.getObjectHandle("dr20_dynamicRightWheel") -- Handle of the right motor
    noseSensor=sim.getObjectHandle("SensingNose") -- Handle of the proximity sensor
    cycleRed=sim.getObjectHandle("redCycle")
    cycleYellow=sim.getObjectHandle("yellowCycle")
    cycleGreen=sim.getObjectHandle("greenCycle")
    ...
    if (not pluginNotFound) then
        ...
        local redCycleTopicName='redCycle'..sysTime
        local yellowCycleTopicName='yellowCycle'..sysTime
        local greenCycleTopicName='greenCycle'..sysTime
        ...
        cycleRedSub=simROS.subscribe('/'..redCycleTopicName,'std_msgs/Float32','setRedCycle_cb')
        cycleYellowSub=simROS.subscribe('/'..yellowCycleTopicName,'std_msgs/Float32','setYellowCycle_cb')
        cycleGreenSub=simROS.subscribe('/'..greenCycleTopicName,'std_msgs/Float32','setGreenCycle_cb')

        -- Now we start the client application:
        result=sim.launchExecutable('turtlebot2i',leftMotorTopicName.." "..rightMotorTopicName.." "..sensorTopicName.." "..simulationTimeTopicName,0)
    end
    
end

function setRedCycle_cb(msg)
    sim.setObjectSizeValues(cycleRed,{msg.data,msg.data,0})
end

function setYellowCycle_cb(msg)
    sim.setObjectSizeValues(cycleYellow,{msg.data,msg.data,0})
end

function setGreenCycle_cb(msg)
    sim.setObjectSizeValues(cycleGreen,{msg.data,msg.data,0})
end
...
function sysCall_actuation()
    ...
    if not pluginNotFound then
        ...
        simROS.sendTransform(getTransformStamped(robotHandle,'turtlebot2i',-1,'world'))
...
function sysCall_cleanup()
    if not pluginNotFound then
        ...
        simROS.shutdownSubscriber(cycleRedSub)
        simROS.shutdownSubscriber(cycleYellowSub)
        simROS.shutdownSubscriber(cycleGreenSub)
    end
end
